---
title: "Theoretical Background: Radiative Transfer Modelling for Remote Sensing applications"
...
Remote sensing provides us with unprecedented capabilities to observe Earth's ecosystems from above. However, the spectral signals captured by satellite and airborne sensors represent complex interactions between electromagnetic radiation, vegetation structures, and biochemical processes. To unlock the full information content of these signals, we need more than statistical relationships—we need physical understanding of how light interacts with plant canopies.
...
The plate model (@fig-leaf-rtm panels c-e) shows how light propagates through leaf layers. At each interface between materials with different refractive indices (typically air n=1 and cell wall material n≈1.4), light undergoes:

1. **Specular reflection** at the surface (~4% of incident light)
2. **Refraction** into the leaf interior following Snell's law
3. **Absorption** by pigments (e.g., chlorophylls, carotenoids, anthocyanins)
4. **Scattering** at cell walls and air-cell interfaces
5. **Multiple internal reflections** within the mesophyll structure

**PROSPECT Model:**

The most widely used leaf-level RTM is PROSPECT [@jacquemoud1990prospect; @feret2017prospect], which treats the leaf as a stack of N identical layers, each characterized by a refractive index and specific absorption coefficients. The latest model requires as inputs (see the [PROSPECT tutorial](https://jbferet.gitlab.io/prospect/articles/prospect1.html)):

- **N**: Leaf structure parameter (related to mesophyll thickness and compactness)
- **CHL**: Chlorophyll a+b content (μg/cm²)
- **CAR**: Carotenoid content (μg/cm²)  
- **BROWN**: Brown pigments (arbitrary units)
- **EWT**: Equivalent water thickness (g/cm² or cm)
- **LDMC**: Dry matter content (g/cm²)
- **ANT**: Anthocyanin content (μg/cm²) (in PROSPECT-D and later versions)
- **LMA**: leaf mass per area (g/cm²) (only in PROSPECT-D)
- **PROT**: proteins (g/cm²) (in PROSPECT-PRO and later versions)
- **CBC**: carbon based constituents (g/cm²) (in PROSPECT-PRO and later versions)
- **alpha**: maximum incidence angle relative to the normal defining the solid angle of incident light at the surface of leaf (degrees) (in PROSPECT-PRO and later versions)
...
- **PROSPECT-PRO** (2021): Improved protein absorption features [@feret2021prospect]
...
- **Tree crowns**: Individual trees with species-specific architecture and optical properties
...
4. **Atmospheric coupling**: Light exiting canopy, being scattered by atmosphere, and potentially re-entering canopy
...
- **Structural parameters** in case of simple tree parametrization**:** LAI (Leaf Area Index), leaf angle distribution, crown dimensions, tree positions
- **3D Architecture** in case of detailed tree parametrization**:** Tree positions, 3D tree representations (3D objects)
- **Environmental conditions**: Sun angles, atmospheric composition, scene location (longitude, latitude, altitude)
- **Sensor geometry**: View angles, spatial and spectral resolution
...
- **Radiative budget**: Absorbed, transmitted, reflected, and emitted radiation
...
Sometimes a fast, simple model that you can run many times is more valuable than a slower, complex model with slightly better accuracy.
:::
...
- Don't require detailed 3D representations (3D objects) of individual trees
...
The most sophisticated approach explicitly represents 3D tree architecture using detailed geometric meshes or voxelized structures (@fig-model-complexity, right panel). Examples include DART [@gastellu2017dart, @malenovsky2021discrete], LESS [@qi2019less], and Helios++ [@winiwarter2022virtual]. 
...
- Can incorporate terrestrial laser scanning (TLS) data (Helios++) and detailed 3D reconstructed forest scenes (3D objects)
- Suitable for fine-scale studies
