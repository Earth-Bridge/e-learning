...
This tutorial demonstrates SAR tomography (TomoSAR) analysis of tropical forests using P-band data from the AfriSAR 2016 campaign. We'll process multi-baseline SAR data to extract 3D forest structure information and compare results with LiDAR measurements. The analysis covers data preprocessing, covariance matrix computation, tomographic inversion, and comparison against airborne LiDAR forest height and biomass data.
...
**Data link**: [F-SAR P-band TomoSAR data from AfriSAR 2016 campaign](https://drive.google.com/file/d/13m59hn_VznG60dsMuDVCusX8mlj7xGzg/view?usp=sharing)

**Study site**: Lopé National Park, Gabon 

**Data sources**:

- SAR data: <https://earth.esa.int/eogateway/campaigns/afrisar-2016>
- LiDAR data: <https://lvis.gsfc.nasa.gov/Data/Maps/Gabon2016Map.html>

**Contact**: xiao.liu\@tu-dresden.de
:::

## Prerequisites

Before starting, you'll need to install the [required Python packages](https://drive.google.com/file/d/18cnV2tv5PlSHgOdaFzmXbnSwT9cRWh0e/view?usp=sharing) and set up your working environment.
...
These custom functions handle the core TomoSAR processing pipeline. Ensure the [`tomosar_toolbox.py`](https://drive.google.com/file/d/1eVjhHKqqHK4Ms9rOucT8eh3jUndSGkm8/view?usp=sharing) and [`tomosar_plotting.py`](https://drive.google.com/file/d/11GulxuwVdH71_D7K3wEXOPWTi9yP79w0/view?usp=sharing) modules are in your workspace directory.
:::
...
quick_look(slc_id, slc_stack, kz_stack, phase_stack, img_path,
          figsize=(5, 4), dpi=150, fontsize=7)
```
...
insar_quick_look(slc_1_id, slc_2_id, slc_stack, normalized_stack,
                figsize=(6, 4), dpi=150, fontsize=9)
```
...
cov_mat_plot(covariance_matrix, img_path,
            figsize=(4, 4), dpi=150, fontsize=9)
```
...
-   **Capon**: High spatial resolution, low radiometric accuracy
-   **Beamforming**: Low spatial resolution, high radiometric accuracy
:::
...
## Part 7: Relating TomoSAR Reflectivity with Forest Height and Above-Ground Biomass
...
- Multi-baseline SAR data requires careful phase calibration 
- Terrain correction is essential for accurate height estimation 
- Multi-looking balances noise reduction and spatial resolution

**Tomographic Inversion** 

- Capon beamformer provides better vertical resolution than conventional methods 
- Vertical profiles reveal ground and canopy structure

**Comparison Results** 

- TomoSAR profiles are sensitive to different forest height and biomass groups 
- Penetration depth varies with polarization and forest density 
- Biomass estimation benefits from multi-layer information
...
-   **Data quality**: Check for phase calibration and temporal decorrelation
